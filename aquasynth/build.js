...
function build() {
    console.log('Starting single HTML bundle compilation for AquaSynth...');
    ...
    try {
        ...
        const synth = fs.readFileSync(path.join(srcDir, 'synth.js'), 'utf8');
        ...
        // Inline Synth Script
        const synthScriptTag = '<script src="synth.js"></script>';
        if (html.includes(synthScriptTag)) {
            html = html.replace(synthScriptTag, `<script>\n${synth}\n</script>`);
            console.log('✔ Inlined synth.js successfully.');
        } else {
            console.warn('⚠️ synth.js script tag not found in index.html');
        }
